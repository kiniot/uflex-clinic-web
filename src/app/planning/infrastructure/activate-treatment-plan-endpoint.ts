...
export class ActivateTreatmentPlanApiEndpoint extends ErrorHandlingEnabledBaseType {
  constructor(
  ...
  activateTreatmentPlan(id: string): Observable<TreatmentPlanResource> {
    return this.http.post<TreatmentPlanResponse>(`${treatmentPlansApiEndpointUrl}/${id}/activate`, {}).pipe(
      map((response) => this.assembler.toResourceFromResponse(response)),
      catchError(this.handleError('Failed to activate treatment plan')),
    );
  }
}
