...
export class PatientsApiEndpoint extends ErrorHandlingEnabledBaseType {
  constructor(
  ...
  getPatients(): Observable<PatientResource[]> {
    return this.http.get<PatientResponse[]>(patientsApiEndpointUrl).pipe(
      map((responses) =>
        responses.map((response) => this.assembler.toResourceFromResponse(response)),
      ),
      catchError(this.handleError('Failed to load clinic patients')),
    );
  }
}
