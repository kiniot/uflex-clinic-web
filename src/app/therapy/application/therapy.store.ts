...
export class TherapyStore {
  ...
  /**
   * Legacy roster placeholder. Production therapy-session views must read
   * patients from OrganizationStore/API only, never from therapy mocks.
   */
  private readonly patientsSignal = signal<TherapyPatient[]>([]);
  ...
  private metricsFor(command: RegisterExerciseCommand): ExerciseMetric[] {
    const repsLabel =
      command.defaultSets > 1
        ? `${command.defaultSets}×${command.defaultReps} reps`
        : `${command.defaultReps} reps`;

    const primary: ExerciseMetric =
      command.holdDurationSec > 0
