...
export class RegisterDevice extends BaseForm {
  ...
  form = new FormGroup({
    ...
    advertisedName: new FormControl<string | null>(null),
  ...
  protected onOpenScanner() {
    this.messageService.add({
      severity: 'info',
      summary: 'QR Scanner',
      detail: 'Scanner feature coming soon',
    });
  }
  ...
  protected async onSubmit() {
    ...
    this.steps.update((steps) =>
      steps.map((s) => ({ ...s, status: s.id === 'database-sync' ? 'in-progress' : s.status })),
    );

    const value = this.form.getRawValue();
    const command: RegisterDeviceCommand = {
      serialNumber: value.serialNumber,
      advertisedName: value.advertisedName?.trim() || undefined,
    ...
    try {
      ...
      this.messageService.add({
        severity: 'success',
        summary: 'Dispositivo registrado',
        detail: `Kit ${command.serialNumber} registrado exitosamente`,
      });
      this.router.navigate(['/clinic-admin/device']);
    } catch {
      this.messageService.add({
      ...
      this.steps.update((steps) =>
        steps.map((s) => ({ ...s, status: s.id === 'form-validation' ? 'pending' : s.status })),
      );
    }
  }
}
