...
export class DeviceTelemetryApiEndpoint extends ErrorHandlingEnabledBaseType {
  ...
  update(command: UpdateTelemetryCommand): Observable<void> {
    const endpointUrl = buildApiUrl(
      ...
      `${environment.platformProviderDevicesEndpointPath}/${command.deviceId}/telemetry`,
    );
    ...
    return this.http
      .patch<void>(endpointUrl, request)
      .pipe(catchError(this.handleError('Failed to update telemetry')));
  }
}
