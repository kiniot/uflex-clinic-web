...
export class DeviceStore {
  ...
  private readonly fleetMetricsSignal = signal<FleetMetrics>({total: 0, available: 0, assigned: 0, inMaintenance: 0, lowBattery: 0, offline: 0});
  ...
  readonly totalActiveKits = computed(() => this.devices().filter(d => d.status !== 'RETIRED').length);

  readonly syncedThisWeek = computed(() => {
    const now = new Date();
    const weekAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    return this.devices().filter(d => d.lastSyncAt && d.lastSyncAt >= weekAgo).length;
  });
  ...
  updateDeviceStatus(serialNumber: string, status: DeviceStatus): void {
    const command: UpdateDeviceStatusCommand = {serialNumber, status};
    this.deviceStatusApi.updateStatus(command).subscribe({
      next: (updatedDevice) => {
  ...
  calibrateDevice(serialNumber: string, action: 'needs_calibration' | 'validate'): void {
    const command: CalibrationCommand = {serialNumber, action};
    this.deviceCalibrationApi.execute(command).subscribe({
      next: (updatedDevice) => {
        this.devicesSignal.update(devices =>
          devices.map(d => d.serialNumber === serialNumber ? updatedDevice : d)
        );
  ...
  unassignDevice(serialNumber: string): void {
    this.deviceAssignmentApi.unassign(serialNumber).subscribe({
      next: () => {
        this.devicesSignal.update(devices =>
          devices.map(d => {
  ...
  deleteDevice(serialNumber: string): Observable<void> {
    return this.deviceApi.delete(serialNumber).pipe(
      tap(() => {
        this.devicesSignal.update(devices =>
          devices.filter(d => d.serialNumber !== serialNumber)
        );
        this.recomputeFleetMetrics();
      }),
      catchError((err: Error) => {
  ...
  updateTelemetry(serialNumber: string, batteryLevel: number): void {
    const command: UpdateTelemetryCommand = {serialNumber, batteryLevel};
    this.deviceTelemetryApi.update(command).subscribe({
      next: () => {
        this.devicesSignal.update(devices =>
          devices.map(d => {
            if (d.serialNumber !== serialNumber) return d;
